...
async function bootstrap() {
  ...
  if (!hasSupabaseConfig) {
    render();
    showToast("supabase.config.js??Supabase URLê³?anon keyë¥??…ë ¥?˜ì„¸??");
    return;
  }
  ...
  refreshPromise = (async () => {
    try {
      ...
      if (!member) {
        ...
        if (!skipToast) {
          showToast("?€?¥ëœ ?„ë¡œ?„ì„ ???´ìƒ ?¬ìš©?????†ìŠµ?ˆë‹¤.");
        }
        return null;
      }
      ...
      if (!skipToast) {
        showToast(extractErrorMessage(error, "Supabase ?™ê¸°?”ì— ?¤íŒ¨?ˆìŠµ?ˆë‹¤."));
      }
      return null;
    } finally {
      refreshPromise = null;
...
function createRoomTitle(family, room, memberId) {
  ...
  if (room.type === "dm") {
    return getDirectPeer(family, room, memberId)?.name ?? "1:1 ì±„íŒ…";
  }
    ...
    ? invites.map((invite) => `
      <article class="invite-card">
        ...
        <div class="message-card-footer">
          ...
          <button class="ghost-button compact" type="button" data-copy-invite="${invite.code}">ë³µì‚¬</button>
        </div>
      </article>
    `).join("")
